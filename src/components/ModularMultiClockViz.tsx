...
import {
  runModularMultiClockAsync,
  type ModularMultiClockResultWithBackend,
} from '@/sim/runner-async'
import type { ModularMultiClockConfig } from '@/sim/types'
...
function PairwiseMatrix({ result }: { result: ModularMultiClockResultWithBackend }) {
...
type RunConfig = ModularMultiClockConfig & { preset: PresetKey; lattice: LatticeKey }
...
export function ModularMultiClockViz() {
  const [config, setConfig] = useState<RunConfig>(DEFAULT)
  const [result, setResult] = useState<ModularMultiClockResultWithBackend | null>(null)
  ...
  const run = useCallback(() => {
    ...
    void runModularMultiClockAsync({
      ...runConfig,
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Modular-flow clock network</CardTitle>
        <CardDescription>
          Same N-clock network as the Z-threshold demo, but each site clock ticks
          from cumulative modular entropy flow (ΔS on single-site regions) rather
          than Z-signal thresholds. Relational time predicts the network still
          lacks a globally consistent time coordinate.
        </CardDescription>
      </CardHeader>
            ...
            <div className="grid lg:grid-cols-2 gap-6 items-start">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Row/column 0 is the uniform Δt clock. Modular-flow clocks use
                  entropy-threshold ticks; defect site should track uniform while
                  distant sites disagree mid-quench.
                </p>
              </div>
